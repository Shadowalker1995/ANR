...
# Dataset & Model
parser.add_argument("-d", dest="dataset", type=str, default="musical_instruments",
                    help="Dataset for Running Experiments (Default: musical_instruments)")
parser.add_argument("-m", dest="model", type=str, default="ANR",
                    help="Model Name, e.g. DeepCoNN|DAttn|ANR|ANRS (Default: ANR)")
...
parser.add_argument("-opt", dest="optimizer", type=str, default="Adam",
                    help="Optimizer, e.g. Adam|RMSProp|SGD (Default: Adam)")
parser.add_argument("-loss_func", dest="loss_function", type=str, default="MSELoss",
                    help="Loss Function, e.g. MSELoss|L1Loss (Default: MSELoss)")
...
parser.add_argument("-WED", dest="word_embed_dim", type=int, default=300,
                    help="Number of Dimensions for the Word Embeddings (Default: 300)")
parser.add_argument("-p", dest="pretrained_src", type=int, default=1,
                    help="Source of Pretrained Word Embeddings? \
                         0: Randomly Initialized (Random Uniform Dist. from [-0.01, 0.01]), \
                         1: w2v (Google News, 300d), 2: GloVe (6B, 400K, 100d) (Default: 1)")

# ANR Hyperparameters
parser.add_argument("-K", dest="num_aspects", type=int, default=5,
                    help="Number of Aspects (Default: 5)")
parser.add_argument("-h1", dest="h1", type=int, default=10,
...
parser.add_argument("-ARL_path", dest="ARL_path", type=str, default="",
                    help="Specify the file name for loading pretrained ARL weights! (Default: "", i.e. Disabled)")
parser.add_argument("-ARL_lr", dest="ARL_lr", type=float, default=0.01,
                    help="RATIO of LR for fine-tuning the pretrained ARL weights (Default: 0.01)")

# Miscellaneous
parser.add_argument("-rs", dest="random_seed", type=int, default=1337,
...
parser.add_argument("-sm", dest="save_model", type=str, default="",
...
def evaluate(mdl, set_loader, epoch_num=-1, use_cuda=True, phase="Dev", print_txt=True):
    ...
    for batch_num, (batch_uid, batch_iid, batch_rating) in enumerate(set_loader):
        ...
        rating_pred = torch.squeeze(mdl(batch_uid, batch_iid))      # mdl: a module, mdl(*args) will call its forward() function
    ...
    logger.log("[{}] {:6s} MSE: {:.5f}, MAE: {:.5f}".format(
...
epoch_num_forBestDevMSE, bestDevMSE, testMSE_forBestDevMSE, testMAE_forBestDevMSE = getBestPerf(lstDevMSE, lstTestMSE,
                                                                                                lstTestMAE)

...
class DAttn_RatingPred(nn.Module):
    def __init__(self, logger, args, num_users, num_items):
        super(DAttn_RatingPred, self).__init__()
    ...
    '''
    [Input]    out_user:    bsz x output_size
    [Input]    out_item:    bsz x output_size
    [Input]    batch_uid:   bsz
    [Input]    batch_iid:   bsz
    [Output]   rating_pred: bsz x 1
    '''
    def forward(self, out_user, out_item, batch_uid, batch_iid, verbose=0):
        if verbose > 0:
            tqdm.write("\n\n**************************************** DAttn Rating Predictor ****************************************")
            tqdm.write("[Input] out_user: {}".format(out_user.size()))              # bsz x output_size
            tqdm.write("[Input] out_item: {}".format(out_item.size()))              # bsz x output_size
        ...
        rating_pred = torch.sum(torch.mul(out_user, out_item), 1, keepdim=True)
        if verbose > 0:

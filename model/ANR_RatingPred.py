...
class ANR_RatingPred(nn.Module):
    ...
    def __init__(self, logger, args, num_users, num_items):
        ...
        self.globalOffset = nn.Parameter(torch.Tensor(1), requires_grad = True) # b(0)

        # User Offset/Bias & Item Offset/Bias
        self.uid_userOffset = nn.Embedding(self.num_users, 1) # b(u)
        self.uid_userOffset.weight.requires_grad = True

        self.iid_itemOffset = nn.Embedding(self.num_items, 1) # b(i)
    ...
    def forward(self, userAspRep, itemAspRep, userAspImpt, itemAspImpt, batch_uid, batch_iid, verbose = 0):
        ...
        for k in range(self.args.num_aspects):
            ...
            aspRating = torch.matmul(user, item) # Formula (8), part2
            if(verbose > 0 and k == 0):
        ...
        rating_pred = userAspImpt * itemAspImpt * rating_pred # Formula (8), part1 multi part2
        if(verbose > 0):
        ...
        rating_pred = torch.sum(rating_pred, dim = 1, keepdim = True) # Formula (8), part1 multi part2
        if(verbose > 0):
        ...
        # Include Global Bias
        rating_pred = rating_pred + self.globalOffset # Formula (8)
        if(verbose > 0):

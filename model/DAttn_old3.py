...
class DAttn(nn.Module):
    ...
    def forward(self, batch_uid, batch_iid, verbose=0):
        ...
        if verbose > 0:
            tqdm.write("\nlocal1_user: {}".format(local1_user.size()))                  # bsz x max_vis_len
            tqdm.write("local2_user: {}".format(local2_user.size()))                    # bsz x max_vis_len
            tqdm.write("local3_user: {}".format(local3_user.size()))                    # bsz x max_vis_len
            tqdm.write("local1_item: {}".format(local1_item.size()))                    # bsz x max_vis_len
            tqdm.write("local2_item: {}".format(local2_item.size()))                    # bsz x max_vis_len
            tqdm.write("local3_item: {}".format(local3_item.size()))                    # bsz x max_vis_len
        ...
        if verbose > 0:
            tqdm.write("\nglobal_user: {}".format(global_user.size()))                  # bsz x max_vis_len
            tqdm.write("global_item: {}".format(global_item.size()))                    # bsz x max_vis_len
        ...
        if verbose > 0:
            tqdm.write("\nout_user: {}".format(out_user.size()))                        # bsz x (4 x max_vis_len)
            tqdm.write("out_item: {}".format(out_item.size()))                          # bsz x (4 x max_vis_len)
...
class LocalAttention(nn.Module):
    def __init__(self, args, input_size, embed_size, win_size, out_channels, hidden_size, output_size):
        ...
        self.attention_layer1 = nn.Sequential(
            # bsz x 1 x (max_vis_len + (win_size - 1) / 2) x word_embed_dim(1) -> bsz x 1 x max_vis_len x 1
    ...
    def forward(self, x):
        ...
        scores1 = self.attention_layer1(x_pad1)                     # bsz x 1 x max_vis_len x 1
        scores1 = scores1.squeeze(1)                                # bsz x max_vis_len x 1

        scores2 = self.attention_layer2(x_pad2)                     # bsz x 1 x max_vis_len x 1
        scores2 = scores2.squeeze(1)                                # bsz x max_vis_len x 1

        scores3 = self.attention_layer3(x_pad3)                     # bsz x 1 x max_vis_len x 1
        scores3 = scores3.squeeze(1)                                # bsz x max_vis_len x 1

        out1 = torch.mul(x, scores1)                                # bsz x max_vis_len x 1
...
class GlobalAttention(nn.Module):
    ...
    '''
    def forward(self, x):
        x = x.unsqueeze(1)                                          # bsz x 1 x max_vis_len x word_embed_dim(1)

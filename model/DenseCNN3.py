...
class DenseNet(nn.Module):
    ...
    def reset_para(self):
        convs = [self.dense_layer_conv0[0], self.dense_layer_conv1[1], self.dense_layer_conv2[1],
    ...
    def forward(self, batch_DocEmbed):
        ...
        fea0 = torch.sum(torch.mul(out0, score0), dim=2)
        out0 = self.dropout(out0)

        # bigram
        # bsz x filters_num x max_doc_len x output_size -> bsz x filters_num x max_doc_len x output_size
        out1 = self.dense_layer_conv1(out0)
        score1 = F.softmax(out1, dim=2)
        # bsz x filters_num x max_doc_len x output_size -> bsz x filters_num x output_size
        fea1 = torch.sum(torch.mul(out1, score1), dim=2)
        out1 = self.dropout(out1)

        # trigram
        # bsz x 2*filters_num x max_doc_len x output_size
        out2 = torch.cat((out1, out0), dim=1)
        # bsz x 2*filters_num x max_doc_len x output_size -> bsz x filters_num x max_doc_len x output_size
        out2 = self.dense_layer_conv2(out2)
        score2 = F.softmax(out2, dim=2)
        # bsz x filters_num x max_doc_len x output_size -> bsz x filters_num x output_size
        fea2 = torch.sum(torch.mul(out2, score2), dim=2)
        ...
        # bsz x 2*filters_num x max_doc_len x output_size -> bsz x filters_num x max_doc_len x output_size
        out3 = self.dense_layer_conv3(out3)
        score3 = F.softmax(out3, dim=2)
        # bsz x filters_num x max_doc_len x output_size -> bsz x filters_num x output_size
        fea3 = torch.sum(torch.mul(out3, score3), dim=2)
        ...
        # bsz x 2*filters_num x max_doc_len x output_size -> bsz x filters_num x max_doc_len x output_size
        out4 = self.dense_layer_conv4(out4)
        score4 = F.softmax(out4, dim=2)
        # bsz x filters_num x max_doc_len x output_size -> bsz x filters_num x output_size
        fea4 = torch.sum(torch.mul(out4, score4), dim=2)

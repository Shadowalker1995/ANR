...
class DenseNet(nn.Module):
    def __init__(self, logger, args, filters_size=None):
        ...
        # bsz x 1 x max_doc_len x output_size -> bsz x 1 x max_doc_len x output_size
        self.dense_layer_conv1 = nn.Sequential(
            nn.ZeroPad2d((0, 0, 0, 1)),
            nn.Conv2d(1, 1, kernel_size=(2, 1)),
            nn.ReLU())
        # bsz x 2 x max_doc_len x output_size -> bsz x 1 x max_doc_len x output_size
        self.dense_layer_conv2 = nn.Sequential(
            nn.ZeroPad2d((0, 0, 0, 1)),
            nn.Conv2d(2, 1, kernel_size=(2, 1)),
            nn.ReLU())
        # bsz x 2 x max_doc_len x output_size -> bsz x 1 x max_doc_len x output_size
        ...
        # bsz x 1 x output_size x 5 -> bsz x 1 x 1 x 5
    ...
    def reset_para(self):
        convs = [self.attention_layer_conv0[0], self.attention_layer_conv1[0], self.attention_layer_conv2[0],
                 ...
                 self.dense_layer_conv1[1], self.dense_layer_conv2[1], self.dense_layer_conv3[1], self.dense_layer_conv4[1],
                 self.scale_attention[0]]
        for conv in convs:
    ...
    def forward(self, batch_DocEmbed):
        ...
        score0 = F.softmax(score0, dim=2)
        # bsz x 1 x max_doc_len x output_size * bsz x 1 x max_doc_len x 1 -> bsz x 1 x max_doc_len x output_size
        out0 = torch.mul(batch_DocEmbed, score0)
        # bsz x 1 x max_doc_len x output_size -> bsz x output_size
        fea0 = torch.sum(out0, dim=2).squeeze(1)
        out0 = self.dropout(out0)     # bsz x 1 x max_doc_len x output_size
        ...
        # bsz x 1 x max_doc_len x output_size -> bsz x output_size
        fea1 = torch.sum(out1, dim=2).squeeze(1)
        out1 = self.dropout(out1)     # bsz x 1 x max_doc_len x output_size

        # trigram
        # bsz x 2 x max_doc_len x output_size
        ...
        # bsz x 1 x max_doc_len x output_size -> bsz x output_size
        fea2 = torch.sum(out2, dim=2).squeeze(1)
        out2 = self.dropout(out2)     # bsz x 1 x max_doc_len x output_size
        ...
        # bsz x 1 x max_doc_len x output_size -> bsz x output_size
        fea3 = torch.sum(out3, dim=2).squeeze(1)
        out3 = self.dropout(out3)     # bsz x 1 x max_doc_len x output_size

        # fivegram
        # bsz x 2 x max_doc_len x 1
        ...
        # bsz x 1 x max_doc_len x output_size -> bsz x output_size
        fea4 = torch.sum(out4, dim=2).squeeze(1)

        # bsz x output_size -> bsz x output_size x 5
        fea = torch.stack((fea0, fea1, fea2, fea3, fea4), dim=2)

        # bsz x output_size x 5 -> bsz x 1 x output_size x 5
        # bsz x 1 x output_size x 5 -> bsz x 1 x 1 x 5 -> bsz x 1 x 5
        scale_score = self.scale_attention(fea.unsqueeze(1)).squeeze(1)
